  ...
  const getErrorMessage = () => {
    if (loginMutation.error) {
      if (isAxiosError<ApiError>(loginMutation.error) && loginMutation.error.response) {
        return loginMutation.error.response.data.error || 'Invalid username or password.';
      }
  ...
  return (
      <div className="p-8 max-w-md w-full bg-white/40 backdrop-blur-md rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md focus:outline-none focus:shadow-outline disabled:bg-blue-300" disabled={loginMutation.isPending}>
